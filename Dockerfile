...
COPY scripts/write-openclaw-plugin-sdk-compat.mjs ./scripts/write-openclaw-plugin-sdk-compat.mjs
...
ARG OPENCLAW_PRUNE_RUNTIME=""
RUN if [ -n "$OPENCLAW_PRUNE_RUNTIME" ]; then \
...
COPY --chown=node:node SOUL.md USER.md AGENTS.md TOOLS.md ORCHARD_WRITE_PROTOCOL.md FARM.md ./
...
# Install additional system packages needed by default (rclone for R2, fuse for VFS, rsync for skill sync, awscli for manifest uploads).
RUN --mount=type=cache,id=openclaw-bookworm-apt-cache,target=/var/cache/apt,sharing=locked \
    --mount=type=cache,id=openclaw-bookworm-apt-lists,target=/var/lib/apt,sharing=locked \
    apt-get update && \
    DEBIAN_FRONTEND=noninteractive apt-get install -y --no-install-recommends \
